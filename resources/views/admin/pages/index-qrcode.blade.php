...
@if (!request()->ajax())
    ...
    <script>
        var urlSequence = '{{ route($type . '-qrcode') }}';
        $('.btn-custom-header').append(
            '<a href="' + urlSequence +
            '" target="_blank" style="margin-left:10px;" class="btn btn-default btn-show-qrcode btn-rounded"><i class="icon-qrcode position-left"></i>Cetak QRcode</a>'
        )
    </script>
    @if (!request()->ajax())
    @endpush
